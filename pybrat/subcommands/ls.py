...
class ListProject(Subcommand):
    ...
    name = "ls"
    description = "List all pybrat projects, virtualenvs or pythons."
    help = "list pybrat projects, virtualenvs or pythons"


    def __init__(self):
        super(ListProject, self).__init__()
#        self.parser.add_argument("-f", "--filter", action="store",
#                                 help="list projects filtered by keyword" )
        self.arg_group = self.parser.add_mutually_exclusive_group()
        self.arg_group.add_argument("-v", "--venv", 
                                    help="list ONLY known virtualenvs")
        self.arg_group.add_argument("-p", "--py", action="store_true",
                                    help="list ALL known python versions")
...
########333
        proj_d = get_projects_dict(args)    # ELIM args_d & changed name from get_project_list()
        if not proj_d:
